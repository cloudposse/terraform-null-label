...
| <a name="input_label_order"></a> [label\_order](#input\_label\_order) | The order in which the ID elements (labels) appear in the `id`.<br>Defaults to ["namespace", "environment", "stage", "name", "attributes"].<br>You can omit any of the 6 label elements ("tenant" is the 6th), but at least one must be present. | `list(string)` | `null` | no |
